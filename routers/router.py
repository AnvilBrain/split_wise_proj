...
async def add_member_to_group(group_id: int, member: AddMember, current_user=Depends(get_current_user), db=Depends(get_db)):
...
async def delete_member_from_group(group_id: int, member: AddMember, current_user=Depends(get_current_user), db=Depends(get_db)):
    await check_access(current_user , db, group_id)
